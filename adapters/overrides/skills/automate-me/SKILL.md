---
name: automate-me
description: "Create or update a personal mode skill from the user's working conventions, supplied examples, and optional project-scoped transcripts. Use for automate me, update my mode skill, or capture my preferences or working style into a skill."
...
1. **Find the existing skill.** Check the installed skill catalog and `{{SKILLS_DIR}}/` for the user's existing mode skill. Read a supplied or discovered user-level skill path when applicable. A request to update authorizes editing that skill; preserve its location and unaffected conventions. For a new skill, default to `{{SKILLS_DIR}}/<handle>-mode/SKILL.md` in this project. Use the user's chosen handle when known; ask for one if needed to name the artifact.
