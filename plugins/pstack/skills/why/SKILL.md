...
Before spawning investigators, discover the tools and connectors actually exposed by this host. Use its tool inventory, connector search, or documented MCP discovery mechanism. Do not assume a Cursor `mcps/` directory exists. Classify only capabilities you can call; record unavailable categories as gaps.
...
- Read-only investigation. Use available search/read connector operations and do not mutate files, tickets, chats, or external systems. If delegation lacks a necessary connector, the parent performs that read-only lookup and passes the evidence. Do not broaden permissions to recover connector access.
...
- Read-only synthesis. Spot-verify citations using available read operations. If the subagent lacks a connector, ask the parent to perform the lookup and return evidence. Do not grant write access for a read task.
