...
- Read another skill through the host's installed skill catalog when available. Otherwise resolve `../<skill-name>/SKILL.md` from the current skill directory. Project installations live under `.agents/skills/`; user installations may be elsewhere, so prefer the discovered absolute path.
