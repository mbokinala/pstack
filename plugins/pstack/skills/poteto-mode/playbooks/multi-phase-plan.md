...
3. Explore in subagents with `role: "poteto-agent"` and an explicit model per the Subagents section (the **guard-the-context-window** principle skill). Each returns file pointers, conventions, test commands, and entry points. No inlined dumps.
...
- [ ] On the operator's go, record the program objective in durable state. Include the plan path, PR ids in order, verification rule, who merges, and the done condition. Use a native goal only when the host exposes it and the user has authorized it.
- [ ] Read these from the installed skill distribution at program start and each audit. Record its installed version. Do not fetch skill files from the consumer repository's trunk or update the distribution mid-run.
  - [ ] The selected execution playbook under the installed poteto-mode skill.
  - [ ] The installed swarm skill.
  - [ ] The actual project verification skill or available control tool instructions.
  - [ ] The installed poteto-mode opening-a-pr playbook.
  - [ ] Each other installed leaf skill used by the program.
- [ ] Record the next audit time, approximately 30 minutes while the session is active. Use native status/wait tools within the host's limits. Use durable scheduling only when exposed and authorized; otherwise record the session-continuity limitation and save a resume checkpoint before stopping.
- [ ] At each audit, re-read the installed execution playbook and recorded objective. Check every active lane against its artifacts and liveness. Stop and confirm termination of a stuck writer before replacement. Report PR, owner, state, head SHA, new verdicts, merges, operator gates, and blockers in the active conversation.
...
Each live lane runs at the PR head in an isolated worktree or an explicitly available authorized remote environment. Use separate ports and data directories for concurrently driven applications; serialize lanes when safe isolation is unavailable. Drive through the actual project verification harness or available control tools. Run the ten scenarios in batches within host limits; ten scenarios do not require ten simultaneous VMs.

- [ ] Fetch `<head-branch>`, create the lane's separate worktree at `<head SHA>`, and verify its HEAD before driving. Never switch or reset a shared dirty checkout.
...
**Verify, live.** Tests alone are not sufficient verification. A PR is verified only when its unit, live, and perf boxes are all checked. Ten verification scenarios at the PR head, per the boot recipe.
